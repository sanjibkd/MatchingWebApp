...
public class ProjectController extends Controller {
	...
	public static Result blockMultiAttribute(String projectName) {
		DynamicForm form = form().bindFromRequest();
		Logger.info("PARAMETERS : " + form.data().toString());
		String table1Name = form.get("table1_name");
		String table2Name = form.get("table2_name");
		List<String> bmAttribute1Names = Utils.getFormArrayParameters(form, "attr1_", 1);
		List<String> bmAttribute2Names = Utils.getFormArrayParameters(form, "attr2_", 1);
		String[] table1BlockingAttributeNames = bmAttribute1Names.toArray(new String[0]);
		String[] table2BlockingAttributeNames = bmAttribute2Names.toArray(new String[0]);
		String candsetName = form.get("candset_name");
		String[] table1AttributeNames = request().body().asFormUrlEncoded().get("attr1_names[]");
		String[] table2AttributeNames = request().body().asFormUrlEncoded().get("attr2_names[]");
		...
		try {
			Table candset = BlockingDao.block(projectName, table1Name,
					table2Name, table1BlockingAttributeNames,
					table2BlockingAttributeNames, candsetName,
	...
	public static Result deleteProject(String projectName) {
		try {
			...
			statusMessage = String.format("Successfully deleted Project %s.", projectName);
			return redirect(controllers.project.routes.ProjectController.index());
		}
		catch(IOException ioe) {
			statusMessage = "Error: " + ioe.getMessage();
	...
	public static Result javascriptRoutes() {
		...
		return ok(
				Routes.javascriptRouter("projectJsRoutes",
						...
						controllers.project.routes.javascript.RuleController.getAttributesForFeature(),
						controllers.project.routes.javascript.RuleController.getFunctionForFeature(),
						controllers.project.routes.javascript.RuleController.getTermsForRule(),
						controllers.project.routes.javascript.RuleController.getAllRuleNames(),
						controllers.project.routes.javascript.RuleController.getRulesForMatcher()
						)
				);
	}
}
